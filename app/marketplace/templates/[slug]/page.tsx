...
export async function generateMetadata({
  ...
  if (!template) {
    return {
      title: "Template Marketplace | Childbook.ai",
      description:
        "Browse production-ready, personalized children's book templates from Childbook.ai.",
    };
  }

  const title = `${template.title} | Childbook Templates`;
  const description = template.excerpt || template.description || undefined;
  return {
    title,
    description: description ?? undefined,
    keywords: template.keywords?.length
      ? [...template.keywords, template.category, "Childbook.ai"]
      : undefined,
    alternates: { canonical: `/marketplace/templates/${template.slug}` },
    openGraph: {
      title,
      description: description ?? undefined,
      type: "website",
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_360px]">
            <div className="space-y-6">
              <section className="rounded-[32px] bg-white p-6 ring-1 ring-blue-1000/10 sm:p-8">
                <div className="inline-flex items-center gap-2 rounded-full border-2 border-blue-1000/15 bg-blue-100 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-blue-800">
                  <Sparkles className="h-3 w-3" aria-hidden />
                  Ready for branded personalization
                </div>
                <h1 className="mt-4 text-3xl font-bold leading-tight text-blue-1000 sm:text-4xl md:text-[2.75rem]">
                  {template.title}
                </h1>
                {template.excerpt ? (
                ...
                <dl className="mt-6 grid gap-3 sm:grid-cols-3">
                  <SnapshotItem
                    icon={Users}
                    label="Audience"
                    value={template.age ?? "All ages"}
                  />
                  <SnapshotItem
                    icon={BookOpen}
                    label="Length"
                    value={
                      template.pages ? `${template.pages} pages` : "Variable"
                    }
                  />
                  <SnapshotItem
                    icon={Languages}
                    label="Language"
                    value={template.language?.toUpperCase() ?? "EN"}
                  />
                </dl>
              </section>
              ...
              {spreads.length > 0 ? (
                <section className="rounded-[32px] bg-white p-6 ring-1 ring-blue-1000/10 sm:p-8">
                  ...
                  <p className="mt-1 text-sm text-blue-1000/70">
                    A taste of the artwork your customers will receive.
                  </p>
                  <div className="mt-5">
                    <PreviewGallery
                      spreads={spreads}
              ...
              <section className="rounded-[32px] bg-white p-6 ring-1 ring-blue-1000/10 sm:p-8">
                ...
                <p className="mt-1 text-sm text-blue-1000/70">
                  Choose how much creative control you want when delivering
                  the book.
                </p>
                <div className="mt-5">
                  <TierSummary
                    availableTiers={
                      template.availableTiers as MarketplaceTier[] | null
                    }
                  />
                </div>
              </section>
...
async function safeFetch(slug: string) {
  try {
    ...
    console.error("[marketplace] template fetch failed", { slug, error });
    return null;
  }
}
